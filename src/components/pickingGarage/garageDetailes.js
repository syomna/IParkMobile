...
const GaragDetails = React.forwardRef(({id} , ref)=> {
  ...
  if (garage.length <= 0) return <View><Text>Loading ....</Text></View>
  return <GestureDetector gesture={gesture}>
    <Animated.View style={[styles.bottomSheetContainer, reStyleBottomSheet]}>
    <View style={styles.line} />
    <View style={{justifyContent: 'space-between' }}>
            <Text style={{color:'#000', fontSize:20}}>{garage[0].garage['name']}</Text>
            </View>
            <Paragraph>
          ...
          <View style={{ backgroundColor: '#bfbcbc38', padding:20, marginTop:30 }}>
            <View style={{ flexDirection: 'row' }}>
              <View style={{ flex: 1, alignItems: 'center', borderRightWidth: 1, borderRightColor: 'gray', paddingVertical: 5 }}>
              ...
              <View style={{ flex: 1, alignItems: 'center', paddingVertical: 5 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <Text style={{ fontWeight: '600', color:'#000', fontSize:20 }}>2 mins</Text>
                </View>
        ...
        <Tab.Item style={{borderColor:'#000'}}  titleStyle={{color:'#AA23B6'}}>Information</Tab.Item>
        <Tab.Item titleStyle={{color:'#AA23B6'}}>Reviews</Tab.Item>
        <Tab.Item titleStyle={{color:'#AA23B6'}}>How to park</Tab.Item>
      </Tab>
      <View style={index === 0 ? {height:450} : styles.d_none}>
          <Text style={styles.description}>{garage[0].garage['description']}</Text>
          <View style={{display:'flex', flexDirection:'row', width:'100%', justifyContent: 'space-around', marginVertical:50}}>
          <Image 
        ...
        <View style={index === 1 ? {height:450} : styles.d_none}>
          <Text style={styles.description}>There are no reviews</Text>
          
        </View>
        <View style={index === 2 ? {height:450} : styles.d_none}>
        <View style={styles.container}>
      <View style={styles.item}>
        <Image source={svg1} style={[styles.imag, {marginTop:25}]} />
        <View style={styles.itemContent}>
          <Text style={styles.boldText}>Once you've paid</Text>
          <Text style={{color:'#000', fontSize:20, marginTop:10}}>You'll receive directions to the space and information on how to access it</Text>
        </View>
      </View>
      <View style={styles.item}>
        <Image source={svg2} style={styles.imag} />
        <View style={styles.itemContent}>
          <Text style={{color:'#000', fontSize:20}}>The space owner/car park is notified of your booking</Text>
        </View>
      </View>
      <View style={styles.item}>
        <Image source={svg3} style={styles.imag} />
        <View style={styles.itemContent}>
          <Text style={{color:'#000', fontSize:20}}>Just turn up, park your vehicle and get on with your day!</Text>
        </View>
      </View>
    </View>
        </View>
          <TouchableOpacity style={{backgroundColor: "#AA23B6",
          width: "100%", padding:15, borderRadius:10, marginVertical:10}}
          onPress={() => {
            ...
            <Text style={{color:'#fff', textAlign:'center', fontSize:20}}>Proceed to Checkout</Text>
          </TouchableOpacity>          
  </Animated.View>
  </GestureDetector>;
});
